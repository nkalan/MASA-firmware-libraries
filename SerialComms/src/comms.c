...
uint8_t send_data(CLB_send_data_info *info, uint8_t type) {
  ...
  */
  ...
  while (clb_pos < CLB_buffer_sz) {
    ...
    uint16_t transfer_sz_left =
        (clb_sz_left > ping_sz_left) ? ping_sz_left : clb_sz_left;
    ...
    if (type == CLB_Telem) {
      stuffed_packet_sz =
          stuff_packet(CLB_ping_packet, CLB_pong_packet, ping_pos);
      ...
      stuffed_packet_sz =
          stuff_packet(CLB_ping_packet, info->flash_arr + flash_pos, ping_pos);
...
uint8_t receive_data(UART_HandleTypeDef *uartx, uint8_t *buffer,
                     uint16_t buffer_sz) {
  ...
  if (CLB_board_addr == CLB_receive_header.target_addr) {
    ...
    if (CLB_receive_header.packet_type < COMMAND_MAP_SZ) {
      ...
      if (cmd_index != -1 && validate_command(CLB_receive_header.packet_type,
                                              data_sz) == CLB_RECEIVE_NOMINAL) {
...
uint16_t stuff_packet(const uint8_t *unstuffed, uint8_t *stuffed,
                      uint16_t length) {
  ...
  if (CLB_header->do_cobbs) {
    ...
    // holds the position of the last zero to be updated when the next 0 is
    // found
    uint8_t *code_ptr =
        stuffed++; // Note: this sets code_ptr to stuffed, then ++ stuffed
    *code_ptr = 1;
    while (length--) {
      if (*unstuffed) {
        *stuffed++ = *unstuffed++;
...
uint16_t unstuff_packet(uint8_t *stuffed, uint8_t *unstuffed, uint16_t length) {
  ...
  for (; stuffed < end; copy--) {
    if (!*stuffed)
      break; // early return if zero is encountered
    if (copy != 0) {
      *unstuffed++ = *stuffed++;
    } else {
      if (code != 0xFF)
